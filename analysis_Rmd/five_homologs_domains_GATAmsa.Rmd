---
title: "Five key homologs of Gat201"
...
This is a small-scale homology analysis of _Cryptococcus neoformans_ Gat201, along with 4 other landmark homologs.

We plot the domain structure, and show a multiple sequence alignment of the conserved GATA zinc-finger domain.

# Load protein information and identifiers from file

```{r load_protein_info}
protein_info <- 
  here::here("data","five_homolog_info.txt") %>%
  readr::read_tsv() %>%
...
# to expand x-limits so that text labels print inside the plot
xvalue_to_expand_for_text <- with(domain_info_nhrgata,
                                max(Length - domain_start) + 25)
...
ggplot(data = domain_info_nhrgata,
  ...
  geom_segment(aes(x = 1, xend = Length), 
               linewidth = 8, colour = "grey50") +
  geom_segment(aes(x = domain_start, xend = domain_end), 
               linewidth = 8, colour = "red2") +
  expand_limits(x = xvalue_to_expand_for_text) # expanding limits as above
```
...
ggplot(data = domain_info_nhrgata,
  ...
  geom_text(aes(x = 1, label = domain_start), 
            vjust = -1.3, hjust = 0.7, colour = "darkred") +
  geom_text(aes(x = domain_end - domain_start, label = domain_end), 
            vjust = -1.3, hjust = 0.3, colour = "darkred") +
  expand_limits(x = xvalue_to_expand_for_text) # again expanding limits as above
```
...
```{r load_sequences}
protein_seqs <- here::here("data", "five_homolog_sequences_uniprot.fasta") %>% 
...
Extract the NHR/GATA domain sequences, align using Muscle in the msa function.
...
Plot figure of the multiple sequence alignment.
